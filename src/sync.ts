...
export interface VisualEditorHostClient extends VSCodeVisualEditorHost {
  host: JsonRpcPostMessageTarget;
  server: EditorServer;
  services: EditorServices;
}

// json rpc request client
export function visualEditorJsonRpcRequestTransport(host: JsonRpcPostMessageTarget) {
  const { request } = jsonRpcPostMessageRequestTransport(host);
  return request;
}
...
export async function syncEditorToHost(
  ...
  visualEditorHostServer(host.host, {
...
function visualEditorHostServer(rpcTarget: JsonRpcPostMessageTarget, editor: VSCodeVisualEditor) {

  // target for message bus
  const target: JsonRpcPostMessageTarget = {
    postMessage: (data) => {
      rpcTarget.postMessage(data);
    },
